...
import com.hardik.calendarapp.presentation.adapter.TAG
...
suspend fun getAllCursorEvents(context: Context): List<CursorEvent> =
    withContext(Dispatchers.IO) {
        ...
        cursor?.use {
            while (it.moveToNext()) {
                try {
                    ...
                    Log.e(TAG, "getAllCursorEvents: $alertOffset", )
        ...
        val googleEventIndex = events.indexOfFirst { it.title.contains("Google") }
                if (googleEventIndex != -1) {
                    val googleEvent = events[googleEventIndex]
                    Log.e(TAG, "getAllCursorEvents: $googleEvent", )
        }
...
private fun getReminderMinutes(context: Context, eventId: Long): Int {
    ...
    try {
        ...
        Log.e(TAG, "Error fetching reminder for event $eventId: ${e.message}", e)
    }
