package calendar.schedule.task.todo.event.reminder.data.repository

import calendar.schedule.task.todo.event.reminder.common.Constants.apiKey
import calendar.schedule.task.todo.event.reminder.data.remote.api.ApiInterface
...
class HolidayApiRepositoryImpl@Inject constructor(private val apiInterface: ApiInterface) :
    ...
    override suspend fun getHolidayEvents(countryCode: String, languageCode:String): HolidayApiDto {
        ...
        val data = apiInterface.getCalendar(apiKey = apiKey,countryCode = countryCode, languageCode = languageCode, timeMin = pair.first, timeMax = pair.second)
        return data
    }
